...
process(event(V, L), [], Ref2) :-
	assertz(history(event(V, L))),
	parselist(L, Ref2).

% parselist(event(), Ref2)
parselist([actor(A), object(possessive(P,O))], Ref2) :-
	add_thing(O),
	add_thing(A),
	search_in_db(P, Ref2).
parselist([actor(personal(P1)), object(personal(P2))], Ref2) :-
	search_in_db([P1,P2], Ref2).
parselist([actor(A), object(personal(P))], Ref2) :-
	add_thing(A),
	search_in_db(P, Ref2).
parselist([actor(A1), object(thing(O,[object(A2)]))],Ref2) :-
	add_thing(A1),
	add_thing(A2),
	add_to_db(O),
	Ref2 = [].
parselist([actor(S), object(personal(P))], Ref2) :-
	add_set(S),
	search_in_db(P, Ref2).
...
search_in_db(P, [R]) :-
	personal(P, [N|[G]]),
	P = it,
	S = [isa(physical_object), G, N],
	history(thing(R, S)).

% P is a list
search_in_db([P|T], [R|Ref2]) :-
	search_in_db(T, Ref2),
	personal(P, [N|[G]]),
	(P = he; P = she),
	S = [isa(person), G, N],
...
search_in_db([P|T], [R|Ref2]) :-
	search_in_db(T, Ref2),
	(P = they; P = those),
	S = [isa(person), gender(neutral), number(plural)],
	history(thing(R, S)).
search_in_db([P], [R]) :-
	personal(P, [N|[G]]),
	P = it,
...
% add noun to database
add_to_db(N) :-
	thing(N, D),
	asserta(history(thing(N, D))).
add_to_db([A1, A2]) :-
	asserta(history(thing([A1,A2], [isa(person), gender(neutral), number(plural)]))).
